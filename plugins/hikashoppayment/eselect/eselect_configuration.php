...
if(!function_exists('curl_init')) {
	echo '<tr><td colspan="2"><strong>The eSelect payment plugin needs the CURL library installed but it seems that it is not available on your server. Please contact your web hosting to set it up.</strong></td></tr>';
}
curl_init();
?><tr>
	<td class="key">
		<label for="data[payment][payment_params][store_id]">
			Store ID
		</label>
	</td>
	<td>
		<input type="text" name="data[payment][payment_params][store_id]" value="<?php echo @$this->element->payment_params->store_id; ?>" />
	</td>
</tr>
<tr>
	<td class="key">
		<label for="data[payment][payment_params][api_token]">
			API Token
		</label>
	</td>
	<td>
		<input type="text" name="data[payment][payment_params][api_token]" value="<?php echo @$this->element->payment_params->api_token; ?>" />
	</td>
</tr>
<tr>
	<td class="key">
